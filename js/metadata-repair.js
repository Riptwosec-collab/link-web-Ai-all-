...
async function migrateAll(){
  ...
  let count=0;for(const link of queue){await waitVisible();await scanOne(link,cfg);count++;refreshCards();await delay(count%3===0?700:300)}
...
async function manualRefresh(id,button){const link=(await getAll('links')).find(x=>x.id===id);if(!link)return;button.disabled=true;button.innerHTML='<i class="ph ph-circle-notch v9-spin"></i>';try{await scanOne({...link,cardDataVersion:0},await settings());refreshCards()}finally{button.disabled=false;button.innerHTML='<i class="ph ph-arrows-clockwise"></i>'}}
