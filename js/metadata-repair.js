...
const needsRepair=link=>badTitle(link)||!String(link?.description||link?.summary||'').trim()||generic(link?.description)||generic(link?.summary)||!link?.imageUrl;
function score(link){let s=0;const h=hostOf(link?.url||'').toLowerCase(),t=String(link?.title||'').trim().toLowerCase();if(t&&t!==h&&!t.startsWith('xn--'))s+=4;if(String(link?.description||'').trim().length>=35&&!generic(link.description))s+=2;if(String(link?.summary||'').trim().length>=25&&!generic(link.summary))s+=1;if(link?.imageUrl)s+=3;if(link?.favicon)s+=1;return s}
...
async function repairLink(link,{force=false}={}){
 ...
 try{
  ...
  const next={...link,title:m.title||link.title,domain:m.domain||link.domain,description:m.description||link.description||'',summary:m.summary||m.description||link.summary||'',imageUrl:m.imageUrl||link.imageUrl||'',favicon:m.favicon||link.favicon||'',category:m.category||link.category||'General',tags:(m.tags?.length?m.tags:link.tags)||[],metadataSource:m.source||link.metadataSource||'',metadataRefreshedAt:Date.now(),updatedAt:Date.now()};
  const after=score(next);
  if(force||after>before){await putOne('links',next);await logEvent('metadata_refresh',{id:link.id,before,after,source:m.source||'unknown'});return true}
...
async function autoRepair(){
 ...
 const last=Number(localStorage.getItem('slh_metadata_repair_last')||0);if(Date.now()-last<10*60*1000)return;
 localStorage.setItem('slh_metadata_repair_last',String(Date.now()));
 ...
 if(changed&&!sessionStorage.getItem('slh_metadata_repaired_once')){sessionStorage.setItem('slh_metadata_repaired_once','1');location.reload()}
}
...
function addRefreshButtons(){
 document.querySelectorAll('.link-card').forEach(card=>{
  ...
  const btn=document.createElement('button');btn.className='card-action';btn.type='button';btn.dataset.refreshMetadata='1';btn.title='Refresh title, description & preview';btn.setAttribute('aria-label','Refresh metadata');btn.innerHTML='<i class="ph ph-arrows-clockwise"></i>';menu.appendChild(btn);
 });
}

new MutationObserver(addRefreshButtons).observe(document.documentElement,{subtree:true,childList:true});
addRefreshButtons();
