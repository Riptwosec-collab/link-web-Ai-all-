...
const RULES=[
  ...
  {category:'Travel',domains:['booking.com','agoda.com','trip.com','skyscanner.com','airbnb.com','hotels.com','expedia.com','maps.google.com','google.com/maps','traveloka.com'],keywords:['hotel','flight','travel','trip','เที่ยว','ที่พัก','โรงแรม','ตั๋วเครื่องบิน','booking','destination','itinerary'],tags:['Travel']},
...
const domainMatch=(host,rule)=>rule.domains.some(d=>host===d||host.endsWith('.'+d)||host.includes(d));

export function classifyLink(link={}){
  const host=hostOf(link),title=norm(link.title),desc=norm(link.description),summary=norm(link.summary),url=norm(link.url),type=norm(link.contentType),text=`${title} ${desc} ${summary} ${url} ${type}`;
  ...
  if(best.score<2)return {category:'General',confidence:0,tags:[],source:'local-rules-v1',reasons:[]};
  return {...best,confidence,source:'local-rules-v1'};
}
...
export async function autoClassifyLink(linkOrId,{force=false}={}){
  ...
  if(!link)return null;
  if(!force&&!canAutoUpdate(link))return null;
  ...
  const next={
    ...
    tags:mergeTags(link,result),
    autoCategory:result.category,
    autoCategoryConfidence:result.confidence,
    autoCategorySource:result.source,
    autoCategorizedAt:Date.now(),
...
export function initAutoCategory(){
  ...
  window.addEventListener('smartlink:local-mutation',e=>{
    const d=e.detail||{};if(d.store!=='links')return;
    if(d.key)schedule(d.key,450);else schedule(null,700);
  });
