(() => {
  ...
  const styles = `
    #smartlink-auth-gate{position:fixed;inset:0;z-index:999999;display:grid;place-items:center;padding:24px;background:radial-gradient(circle at 18% 20%,rgba(34,211,238,.14),transparent 28%),radial-gradient(circle at 82% 82%,rgba(139,92,246,.16),transparent 30%),rgba(3,5,10,.97);backdrop-filter:blur(28px);font-family:Inter,system-ui,sans-serif}
    #smartlink-auth-gate.hidden{display:none}
    .slh-login{position:relative;width:min(430px,100%);overflow:hidden;border-radius:30px;border:1px solid rgba(255,255,255,.09);background:linear-gradient(145deg,rgba(20,24,36,.88),rgba(8,10,17,.95));box-shadow:0 35px 120px rgba(0,0,0,.58),0 0 70px rgba(34,211,238,.06);padding:32px}
    ...
    .slh-submit{width:100%;height:50px;margin-top:20px;border:0;border-radius:15px;color:white;font:600 13px Inter,system-ui;cursor:pointer;background:linear-gradient(100deg,#2563eb,#0891b2);box-shadow:0 12px 34px rgba(37,99,235,.22);transition:.2s}.slh-submit:hover{transform:translateY(-1px);filter:brightness(1.08)}.slh-submit:disabled{opacity:.55;cursor:wait;transform:none}
  ...
  function ensureGate(){
    ...
    gate.innerHTML=`<div class="slh-login">
      ...
      <p>Sign in to unlock your saved links, collections, analytics and cloud-ready workspace.</p>
      <form id="smartlink-login-form">
        ...
        <div class="slh-field"><label>PIN</label><input name="pin" type="password" inputmode="numeric" pattern="[0-9]{6}" maxlength="6" autocomplete="current-password" placeholder="••••••" required></div>
        <button class="slh-submit" type="submit">Sign in securely</button>
  ...
  async function validate(token){
    ...
    try{
      const rows=await rpc('smartlink_session',{p_token:token});
  ...
  async function login(username,pin){
    ...
    if(!row || row.error_code){
      if(row?.error_code==='try_later') throw new Error('Too many attempts. Try again in 10 minutes.');
  ...
  async function start(){
    ...
    const app=document.getElementById('app'); if(app) app.style.filter='blur(8px)';
    const token=localStorage.getItem(TOKEN_KEY);
    const valid=await validate(token);
    if(valid){
      gate.classList.add('hidden'); if(app) app.style.filter=''; addUserButton(valid); return;
    }
    localStorage.removeItem(TOKEN_KEY); localStorage.removeItem(PROFILE_KEY);
    const form=document.getElementById('smartlink-login-form');
    const status=document.getElementById('smartlink-login-status');
    form.addEventListener('submit',async e=>{
      e.preventDefault(); status.className='slh-status'; status.textContent='Checking secure session…';
      const btn=form.querySelector('button'); btn.disabled=true;
      try{
        const fd=new FormData(form); const profile=await login(String(fd.get('username')||''),String(fd.get('pin')||''));
        status.textContent='Signed in'; gate.classList.add('hidden'); if(app) app.style.filter=''; addUserButton(profile); form.reset();
      }catch(err){status.className='slh-status err';status.textContent=err.message}
