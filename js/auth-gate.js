(() => {
  ...
  const rpc = async (name, body) => {
    ...
    if (!res.ok) {
      ...
      const err=new Error(`Auth service error (${res.status}) ${text.slice(0,180)}`);err.status=res.status;throw err;
    }
    return res.json();
  };
  ...
  function ensureGate(){
    ...
    gate.innerHTML=`<div class="slh-login">
      ...
      <div class="slh-kicker">Private Cloud Library · V6</div>
      <h1>Smart Link Hub</h1>
      <p>เข้าสู่ระบบ Cloud เพื่อให้ทุกการเพิ่ม แก้ไข จัดหมวดหมู่ และลบ Auto Save ไปยัง Supabase พร้อม Version History</p>
      ...
      <div class="slh-foot"><span>Auto Save → Supabase</span><span class="slh-secure">Device-aware · 90-day session</span></div>
    </div>`;
    document.body.appendChild(gate);return gate;
  }

  function closeUserMenu(){document.querySelector('.slh-user-menu')?.classList.remove('open');document.querySelector('.slh-user-button')?.classList.remove('open')}

  function addUserDropdown(profile){
    const host=document.querySelector('header .flex.items-center.gap-2');if(!host||document.getElementById('smartlink-user-wrap'))return;
    const wrap=document.createElement('div');wrap.id='smartlink-user-wrap';const name=profile?.display_name||'Mek';
    wrap.innerHTML=`<button id="smartlink-user-button" class="slh-user-button" type="button" aria-haspopup="menu" aria-expanded="false"><span class="slh-user-dot"></span><span>${name}</span><span class="slh-user-chevron">▾</span></button><div class="slh-user-menu" role="menu"><div class="slh-user-card"><b>${name}</b><small><span class="slh-menu-dot"></span>Cloud connected · Auto Save ON</small><small>${deviceName()}</small></div><button class="slh-menu-action" type="button" data-smartlink-cloud-center><span>☁</span>Cloud & Backup Center</button><button class="slh-menu-action" type="button" data-smartlink-sync><span>↻</span>Sync now</button><button class="slh-menu-action" type="button" data-smartlink-lock><span>⌁</span>Lock</button><button class="slh-menu-action danger" type="button" data-smartlink-logout><span>↪</span>Logout</button></div>`;
    ...
    wrap.querySelector('[data-smartlink-cloud-center]').addEventListener('click',()=>{closeUserMenu();window.dispatchEvent(new Event('smartlink:v6-cloud-center'))});
    ...
    wrap.querySelector('[data-smartlink-lock]').addEventListener('click',()=>{closeUserMenu();window.dispatchEvent(new Event('smartlink:v6-lock'))});
  ...
  async function validate(sessionToken){
    ...
    try{const rows=await rpc('smartlink_session',{p_token:sessionToken});return Array.isArray(rows)&&rows[0]?rows[0]:null}
  ...
  async function login(username,pin){
    let rows;
    try{rows=await rpc('smartlink_login_v2',{p_username:username,p_pin:pin,p_device_name:deviceName()})}
    catch(err){if(err.status===404||/PGRST202|smartlink_login_v2/i.test(err.message))rows=await rpc('smartlink_login',{p_username:username,p_pin:pin});else throw err}
    ...
    const profile={profile_id:row.profile_id,display_name:row.display_name,username,expires_at:row.expires_at,device_name:deviceName()};
    localStorage.setItem(PROFILE_KEY,JSON.stringify(profile));return profile;
  }

  async function logout(){const sessionToken=localStorage.getItem(TOKEN_KEY);try{if(sessionToken)await rpc('smartlink_logout',{p_token:sessionToken})}catch{}localStorage.removeItem(TOKEN_KEY);localStorage.removeItem(PROFILE_KEY);window.dispatchEvent(new Event('smartlink:session-cleared'));location.reload()}

  async function start(){
    const gate=ensureGate(),app=document.getElementById('app');if(app){app.style.filter='blur(8px)';app.style.pointerEvents='none'}
    const sessionToken=localStorage.getItem(TOKEN_KEY),valid=await validate(sessionToken);
    if(valid){gate.classList.add('hidden');if(app){app.style.filter='';app.style.pointerEvents=''}addUserDropdown(valid);setTimeout(()=>emitSessionReady(valid),0);return}
    ...
    form.addEventListener('submit',async e=>{
      e.preventDefault();status.className='slh-status';status.textContent='กำลังเชื่อม Cloud…';const submit=form.querySelector('button[type="submit"]');submit.disabled=true;
      try{const fd=new FormData(form),profile=await login(String(fd.get('username')||''),String(fd.get('pin')||''));status.textContent='Cloud connected · Auto Save ON';gate.classList.add('hidden');if(app){app.style.filter='';app.style.pointerEvents=''}addUserDropdown(profile);emitSessionReady(profile);window.dispatchEvent(new Event('smartlink:cloud-force-sync'));form.elements.pin.value=''}
      catch(err){status.className='slh-status err';status.textContent=err.message}finally{submit.disabled=false}
    });
