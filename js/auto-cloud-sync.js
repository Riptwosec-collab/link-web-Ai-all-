...
function mergeStates(local={},cloud={}){const merged={version:6,syncedAt:new Date().toISOString()};for(const store of STORES)merged[store]=mergeRows(store,local?.[store],cloud?.[store]);return applyDeletionRules(merged)}
...
function setStatus(mode,detail=''){lastStatus=mode;lastDetail=detail;const pill=document.getElementById('sync-pill');const map={pending:['syncing',detail||'Queued'],syncing:['syncing',detail||'Saving…'],synced:['cloud',detail||'Cloud saved'],offline:['local',detail||'Saved locally'],conflict:['syncing','Merging…'],error:['local','Cloud retry'],local:['local','Auto Save']};const [cls,label]=map[mode]||map.local;if(pill){pill.className=`status-pill ${cls}`;pill.innerHTML=`<span></span>${label}`;pill.title=mode==='synced'?'Auto Save + conflict-safe Supabase sync is active':mode==='offline'?'Saved locally. Pending changes sync automatically when online.':'Smart Link Hub Auto Save'}emitState(mode,{label})}
