...
function rowTime(row){
  for(const k of ['restoredAt','deletedAt','updatedAt','metadataRefreshedAt','previewReadyAt','capturedAt','createdAt']){
...
function applyDeletionRules(state){
  const tombstones=Array.isArray(state.tombstones)?state.tombstones:[];
  const byEntity=new Map();
  for(const t of tombstones){if(!t?.store||t?.key==null)continue;byEntity.set(`${t.store}:${String(t.key)}`,t)}
  for(const store of ENTITY_STORES){
    state[store]=(Array.isArray(state[store])?state[store]:[]).filter(row=>{
      const k=keyOf(store,row);if(k==null)return false;
      const t=byEntity.get(`${store}:${String(k)}`);if(!t)return true;
      return rowTime(row)>Number(t.deletedAt||0);
    });
  }
  const linksById=new Map((state.links||[]).map(x=>[String(x.id),x]));
  state.trash=(Array.isArray(state.trash)?state.trash:[]).filter(row=>{
    const live=linksById.get(String(row.entityId??row.id));
    return !live||rowTime(live)<=Number(row.deletedAt||0);
  });
  return state;
}
function mergeStates(local={},cloud={}){
  const merged={version:6,syncedAt:new Date().toISOString()};
  for(const store of STORES)merged[store]=mergeRows(store,local?.[store],cloud?.[store]);
  return applyDeletionRules(merged);
}
function canonical(value){
  if(Array.isArray(value))return value.map(canonical);
  if(value&&typeof value==='object'){const out={};for(const k of Object.keys(value).sort())out[k]=canonical(value[k]);return out}
  return value;
}
...
async function rpc(name,body){
  ...
  if(!res.ok){const text=await res.text().catch(()=>String(res.status));throw new Error(`${name} ${res.status}: ${text.slice(0,300)}`)}
  return res.json();
}
function setStatus(mode,detail=''){
  lastStatus=mode;lastDetail=detail;
  const pill=document.getElementById('sync-pill');
  const map={pending:['syncing',detail||'Queued'],syncing:['syncing',detail||'Saving…'],synced:['cloud',detail||'Cloud saved'],offline:['local',detail||'Saved locally'],conflict:['syncing','Merging…'],error:['local','Cloud retry'],local:['local','Auto Save']};
  const [cls,label]=map[mode]||map.local;
  if(pill){pill.className=`status-pill ${cls}`;pill.innerHTML=`<span></span>${label}`;pill.title=mode==='synced'?'Auto Save + conflict-safe Supabase sync is active':mode==='offline'?'Saved locally. Pending changes sync automatically when online.':'Smart Link Hub Auto Save'}
  emitState(mode,{label});
}
async function setPendingStatus(){const n=await getSyncQueueCount().catch(()=>0);if(n)setStatus(navigator.onLine?'pending':'offline',navigator.onLine?`${n} change${n===1?'':'s'} queued`:`${n} waiting`)}
...
async function cloudPut(state,expectedRevision){
  try{
    const rows=await rpc('smartlink_state_put_v2',{p_token:token(),p_state:state,p_expected_revision:Number(expectedRevision||0)});
    return Array.isArray(rows)?rows[0]:rows;
  }catch(err){
    if(/PGRST202|404|smartlink_state_put_v2/.test(String(err.message))){
      const rows=await rpc('smartlink_state_put',{p_token:token(),p_state:state});
      const row=Array.isArray(rows)?rows[0]:rows;return {...row,conflict:false,legacy:true};
    }
    throw err;
  }
}
async function pushState(state,expectedRevision){
  const t=token();if(!t){setStatus('local');return {ok:false}}
  const pending=await getSyncQueueCount().catch(()=>0);
  setStatus('syncing',pending?`Saving ${pending} change${pending===1?'':'s'}…`:'Saving…');
  const row=await cloudPut(state,expectedRevision);
  if(row?.conflict){setStatus('conflict');return {ok:false,conflict:true,revision:Number(row.revision||0)}}
  clearDirty(row?.revision);await clearSyncQueue().catch(()=>{});lastSyncedAt=row?.updated_at||new Date().toISOString();
  const c=counts(state);setStatus('synced',`Cloud saved · ${c.links} links`);
  window.dispatchEvent(new CustomEvent('smartlink:cloud-synced',{detail:{revision:row?.revision||null,updatedAt:lastSyncedAt,...c}}));
  return {ok:true,revision:Number(row?.revision||0)};
}
async function pushNow(){
  if(!token()){setStatus('local');return false}
  if(!navigator.onLine){markDirty();await setPendingStatus();return false}
  let local=await exportCloudState();
  let cloud=await cloudGet();
  let expected=Number(cloud?.revision||0);
  if(cloud?.state){
    local=mergeStates(local,cloud.state);
    if(Number(cloud.revision)!==revision()||fingerprint(local)!==fingerprint(await exportCloudState())){
      restoring=true;try{await importCloudState(local)}finally{restoring=false}
      refreshUi({revision:cloud.revision,...counts(local)});
    }
  }
  let result=await pushState(local,expected);
  if(!result.conflict)return result.ok;

  cloud=await cloudGet();
  if(!cloud)throw new Error('Cloud conflict could not be reloaded');
  local=mergeStates(await exportCloudState(),cloud.state||{});
  restoring=true;try{await importCloudState(local)}finally{restoring=false}
  refreshUi({revision:cloud.revision,conflictResolved:true,...counts(local)});
  result=await pushState(local,Number(cloud.revision||0));
  if(result.conflict)throw new Error('Cloud changed repeatedly during conflict resolution');
  return result.ok;
}
async function drain(){
  ...
  syncing=true;
  try{
    do{requested=false;try{await pushNow()}catch(err){console.warn('Auto cloud sync',err);markDirty();setStatus(navigator.onLine?'error':'offline');break}}while(requested)
  }finally{syncing=false;if(requested&&!debounceTimer){debounceTimer=setTimeout(()=>{debounceTimer=null;drain()},180)}}
}
function queueSync(){
  markDirty();requested=true;setPendingStatus();
  if(restoring||syncing)return;
  clearTimeout(debounceTimer);debounceTimer=setTimeout(()=>{debounceTimer=null;drain()},140);
}
...
async function reconcile(){
  ...
  if(!navigator.onLine){await setPendingStatus();return}
  try{
    const dirty=localStorage.getItem(DIRTY_KEY)==='1';
    const local=await exportCloudState(),cloud=await cloudGet();
    if(!cloud){markDirty();requested=true;await drain();return}
    const merged=mergeStates(local,cloud.state||{});
    restoring=true;try{await importCloudState(merged)}finally{restoring=false}
    refreshUi({revision:cloud.revision,...counts(merged)});
    const cloudNeedsMerge=fingerprint(merged)!==fingerprint(cloud.state||{});
    if(dirty||cloudNeedsMerge||requested){markDirty();requested=true;await drain();return}
    clearDirty(cloud.revision);lastSyncedAt=cloud.updated_at||lastSyncedAt;const c=counts(merged);setStatus('synced',`Cloud saved · ${c.links} links`);
