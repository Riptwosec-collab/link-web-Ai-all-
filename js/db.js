...
let dbPromise;
const CLOUD_STORES=new Set(['links','collections','settings','archives','workspaces']);
function notifyCloud(store,operation,key=null){
  ...
  try{window.dispatchEvent(new CustomEvent('smartlink:local-mutation',{detail:{store,operation,key,at:Date.now()}}))}catch{}
}
...
export async function putOne(name,value){const s=await store(name,'readwrite');return new Promise((res,rej)=>{const r=s.put(value);r.onsuccess=()=>{notifyCloud(name,'put',value?.id??value?.key??null);res(value)};r.onerror=()=>rej(r.error)})}
export async function deleteOne(name,key){const s=await store(name,'readwrite');return new Promise((res,rej)=>{const r=s.delete(key);r.onsuccess=()=>{notifyCloud(name,'delete',key);res()};r.onerror=()=>rej(r.error)})}
export async function clearStore(name){const s=await store(name,'readwrite');return new Promise((res,rej)=>{const r=s.clear();r.onsuccess=()=>{notifyCloud(name,'clear');res()};r.onerror=()=>rej(r.error)})}
export async function bulkPut(name,rows=[]){const db=await openDB();return new Promise((res,rej)=>{const tx=db.transaction(name,'readwrite'),s=tx.objectStore(name);rows.forEach(x=>s.put(x));tx.oncomplete=()=>{notifyCloud(name,'bulk');res()};tx.onerror=()=>rej(tx.error)})}
