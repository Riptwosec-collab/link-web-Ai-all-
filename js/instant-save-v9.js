...
const CARD_DATA_VERSION=11;
const inflight=new Set();
const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));

function toast(text){
  const root=document.getElementById('toast-root');if(!root)return;
  const el=document.createElement('div');el.className='toast v9-save-toast';el.innerHTML=`<i class="ph-fill ph-lightning"></i><span>${esc(text)}</span><b></b>`;root.appendChild(el);setTimeout(()=>el.remove(),2300)
}
...
async function snapshot(link,cfg){if(!cfg.autoSnapshot)return;try{const snap=await makeSnapshot(link,cfg);await putOne('archives',{id:uid('arc'),linkId:link.id,title:link.title,url:link.url,description:link.description,imageUrl:link.imageUrl,capturedAt:Date.now(),...snap});await logEvent('snapshot',{id:link.id,via:'instant-v11'})}catch(err){console.warn('snapshot',err)}}
async function hydrate(id,cfg){
  const original=await getOne('links',id);if(!original)return;
  try{
    const m=cfg.autoMetadata?await getMetadata(original.url,cfg):null;const current=await getOne('links',id);if(!current)return;const now=Date.now();
    ...
    await putOne('links',next);await logEvent('metadata_refresh',{id,source:next.metadataSource,via:'instant-v11',hasBackground:!!(next.heroImageUrl||next.featureImageUrl||next.imageUrl||next.screenshotUrl)});await snapshot(next,cfg);refreshApp();
  }catch(err){console.warn('metadata v11',err);const current=await getOne('links',id);if(current){await putOne('links',{...current,pending:false,previewReadyAt:Date.now(),cardDataVersion:CARD_DATA_VERSION,updatedAt:Date.now()});refreshApp()}}
...
async function instantSave(raw){
  ...
  if(inflight.has(p.normalizedUrl))return;inflight.add(p.normalizedUrl);
  const rows=await getAll('links');if(rows.some(x=>x.normalizedUrl===p.normalizedUrl||String(x.url||'').replace(/\/$/,'')===p.normalizedUrl)){inflight.delete(p.normalizedUrl);return toast('ลิงก์นี้บันทึกไว้แล้ว')}
